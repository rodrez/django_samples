...
export const RateStore = class extends EventTarget {
   ...
   * @private
   */
  _save(options = { dispatchEvent: true }) {
  ...
  addSection(currentYear) {
    ...
    if (yearRangeLength) {
      lastSection = this.rates.yearRanges[this.rates.yearRanges.length - 1];
   ...
   */
  addRow(yearRangeID) {
    const yearRange = this.rates.yearRanges.find((s) => s.id === yearRangeID);
  ...
  setFocusedInput(options) {
    ...
    this._save({ dispatchEvent: false });
  }
  ...
  removeHeader(header) {
    ...
    const index = this.rates.headers.findIndex((header) => header.id === id);
